...
const char* ssid = "Realme PHONE"; //ssid name (wifi name)
const char* password = "Arpit@61299"; // wifi passwords
...
//#define DHTTYPE    DHT22     // DHT 22 (AM2302) Highly efficient
...
void setup(){
  ...
  lcd.setCursor(2,1); // set cursor of lcd to 2, 1 i.e  2nd column and 1st row
  ...
  delay(2000);  // these strings will shown for 2sec only(Cofee n' code Team SIH)
...
// When, you access the route URL, you send the HTML page to the client. 
//In this case, the HTML text is saved on the index_html variable.
...
void loop(){ // loop is predefine which auto call automatically until and unless power is off
  //For DHT11 Sensor
  ...
  lcd.print(random(30,35)); // to print value of body temp. (LM35)
  lcd.print(" C");
  delay(1000); // for delay of 1 sec
Beep(); // calling the beep function for SOS button, if it pushes then it auto on the LED.

// code for emergency buzzer
// For Auto Start the Self made Air Purifier
if (readAQI() > "400"){
  digitalWrite(relayPin, LOW);  // here low means on
}
else {
  digitalWrite(relayPin, HIGH); // high means off
}
}
